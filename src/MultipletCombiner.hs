{- |
Module      :  Physics.MultipletCombining
Copyright   :  (c) Michael Dressel 2022
...
module Phyics.MultipletCombining
    (
...
yt_symbols :: [Int] -> Tableau
yt_symbols [] = Tableau []
yt_symbols is = go (reverse is) (Tableau ["# "])
              where
                go :: [Int] -> Tableau -> Tableau
                go []         t = t
                go (i:is) (Tableau (r:rs)) = go is $ Tableau $
                        concat (r : take i (repeat "# ")) : r : rs
-- | Build multiple tableaux from multiple labels.
yts_symbols :: [[Int]] -> [Tableau]
yts_symbols [] = []
yts_symbols (is:iss) = yt_symbols is : yts_symbols iss
...
yt_nums (Tableau [])  = []
yt_nums (Tableau [l]) = []
yt_nums (Tableau (l:m:ns)) = length l' - length m' : yt_nums (Tableau (m:ns))
                    where l' = noBlank l
                          m' = noBlank m
...
yts_nums :: [Tableau] -> [[Int]]
yts_nums [] = []
yts_nums (t:ts) = case yt_nums t of
                            [] -> yts_nums ts
                            is -> is : yts_nums ts
...
sym2letter (Tableau xss) = Tableau $
                                zipWith (\xs l -> line2let xs l) xss ['a'..]
...
--
--   e.g.: tabs1 (yt_symbols [1,1,1]) "a a "
...
--   yts_nums $ noDoubs.admisTabs $ allTs [1,1] [1,1]
--   [[2,2],[3,0],[0,3],[1,1],[1,1],[0,0]]
--   @

allTs :: [Int] -> [Int] -> [Tableau]
allTs lt rt | (length lt) /= (length rt) = []
            | otherwise = allTsFromSyms (yt_symbols lt)
                ((sym2letter.yt_symbols) rt)
...
admisTabs ts = filter (admis.readTab) ts
...
(><) l r | length l /= length r = []
         | otherwise = yts_nums $ noDoubs.admisTabs $ allTs l r
